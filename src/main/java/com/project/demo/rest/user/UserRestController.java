...
/**
 * Controlador REST para la gestión de usuarios. Proporciona endpoints para operaciones CRUD sobre
 * usuarios y para obtener el usuario autenticado.
 */
@RestController
...
public class UserRestController {
    ...
    /**
     * Obtiene una lista paginada de todos los usuarios. Requiere que el usuario autenticado tenga
     * el rol 'ADMIN' o 'SUPER_ADMIN'.
     *
    ...
    /**
     * Agrega un nuevo usuario al sistema. Requiere que el usuario autenticado tenga el rol 'ADMIN'
     * o 'SUPER_ADMIN'.
     *
    ...
    public ResponseEntity<?> addUser(@RequestBody User user, HttpServletRequest request) {
        ...
        return new GlobalResponseHandler()
                .handleResponse("User updated successfully", user, HttpStatus.OK, request);
    }

    /**
     * Actualiza un usuario existente por su ID. Requiere que el usuario autenticado tenga el rol
     * 'ADMIN' o 'SUPER_ADMIN'.
     *
    ...
    public ResponseEntity<?> updateUser(
            @PathVariable Long userId, @RequestBody User user, HttpServletRequest request) {
    ...
    /**
     * Elimina un usuario por su ID. Requiere que el usuario autenticado tenga el rol 'ADMIN' o
     * 'SUPER_ADMIN'.
     *
    ...
    public ResponseEntity<?> deleteUser(@PathVariable Long userId, HttpServletRequest request) {
        ...
        if (foundOrder.isPresent()) {
            ...
            return new GlobalResponseHandler()
                    .handleResponse(
                            "User deleted successfully", foundOrder.get(), HttpStatus.OK, request);
        } else {
            return new GlobalResponseHandler()
                    .handleResponse(
                            "Order id " + userId + " not found", HttpStatus.NOT_FOUND, request);
        }
    }

    /**
     * Obtiene la información del usuario autenticado actualmente. Requiere que el usuario esté
     * autenticado.
     *
    ...
    /**
     * Restablece la contraseña de un usuario a un valor predeterminado. Requiere que el usuario
     * autenticado tenga el rol 'ADMIN' o 'SUPER_ADMIN'.
     *
     * @param userId El ID del usuario cuya contraseña se va a restablecer.
     * @param request La petición HTTP.
     * @return ResponseEntity con el usuario actualizado y un mensaje de éxito, o un mensaje de
    ...
    public ResponseEntity<?> resetPassword(@PathVariable Long userId, HttpServletRequest request) {
        ...
        return new GlobalResponseHandler()
                .handleResponse(
                        "Password reset successfully to default: " + defaultPassword,
                        user,
                        HttpStatus.OK,
                        request);
    }
    ...
    /**
     * Obtiene un resumen de todos los usuarios. Requiere que el usuario autenticado tenga el rol
     * 'ADMIN' o 'SUPER_ADMIN'.
     *
     * @param request La petición HTTP.
    ...
    public ResponseEntity<?> getUserSummary(HttpServletRequest request) {
        ...
        return new GlobalResponseHandler()
                .handleResponse(
                        "Users summary retrieved successfully",
                        userSummaries,
                        HttpStatus.OK,
                        request);
    }
    ...
    /**
     * Alterna el estado de habilitación de un usuario (enabled/disabled). Requiere que el usuario
     * autenticado tenga el rol 'ADMIN' o 'SUPER_ADMIN'.
     *
    ...
    public ResponseEntity<?> toggleUserEnabled(
        ...
        return new GlobalResponseHandler()
                .handleResponse("User " + status + " successfully", user, HttpStatus.OK, request);
    }
}
