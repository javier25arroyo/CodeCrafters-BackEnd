...
/**
 * Manejador global de respuestas HTTP para la aplicación. Intercepta las respuestas de los
 * controladores para estandarizar el formato de salida, incluyendo metadatos como el método de la
 * petición y la URL.
 */
...
public class GlobalResponseHandler {
     ...
     * @param body El cuerpo de la respuesta, que puede ser cualquier tipo de dato o una instancia
     *     de {@link HttpResponse}.
     ...
     */
    @ResponseBody
    public <T> ResponseEntity<?> handleResponse(
            String message, T body, HttpStatus status, HttpServletRequest request) {
     ...
     * @param status El estado HTTP de la respuesta.
     * @param request La petición HTTP original.
     * @param <T> El tipo genérico (no utilizado en este caso, pero mantiene la consistencia de la
     *     firma).
     * @return Una {@link ResponseEntity} estandarizada.
     */
    @ResponseBody
    ...
    @ResponseBody
    public <T> ResponseEntity<?> handleResponse(
            String message, T body, HttpStatus status, Meta meta) {
        if (body instanceof HttpResponse) {
            HttpResponse<?> response = (HttpResponse<?>) body;
            response.setMeta(meta);
            return new ResponseEntity<>(response, status);
